...
When the UART FIFO receives data it will.   
...
For the RX module whenever it detects the signal being low it would wait half a clock cycle (wait for the signal to peak) and start reading the data into to RX Byte.
